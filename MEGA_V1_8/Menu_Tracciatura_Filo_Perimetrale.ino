...
void Print_LCD_Menu_Tracking(byte LCD_Menu_Tracking) {
  ...
  if (LCD_Menu_Tracking == 2) lcd.print(F("Settaggio PID_Re"));
  if (LCD_Menu_Tracking == 3) lcd.print(F("Base ON/OFF"));
  if (LCD_Menu_Tracking == 4) lcd.print(F("Tracciatura Dir"));
  if (LCD_Menu_Tracking == 5) lcd.print(F("Uscita Zon 1-2"));
  if (LCD_Menu_Tracking == 6) lcd.print(F("Taglio sul filo"));
  if (LCD_Menu_Tracking == 7) lcd.print(F("Trova Filo Av-In")); 
  if (LCD_Menu_Tracking == 8) lcd.print(F("Contatore Ruota"));    
  Max_Options_Tracking = 8;
  }
...
void Activate_Menu_Option_Tracking() {
     ...
     if (Menu_Mode_Selection == 2) {
       ...
       lcd.print(F("PID Tracciatura:"));
       lcd.setCursor(0,1);
       lcd.print(F("P_Return = "));
       lcd.print(P_Return);
       Serial.print(F("Tracking PID P Return = :"));
       Serial.println(P_Return);
       Menu_Complete_Tracking = false;
       while (Menu_Complete_Tracking == false) {
             Read_Membrane_Keys();
             delay(100);
             ...
             lcd.clear();          
             EEPROM.write(177, 1);
             EEPROM.write(176, (P_Return*100));   
             Menu_Mode_Selection = 0;
             }
             if (!Plus_Key_X) {
               P_Return = P_Return + 0.01;
               if (P_Return > 10) P_Return = 10;
               lcd.setCursor(0,1);
               lcd.print(F("      "));    // Fully clear the number to stop reminants of a previous number from being left behind
               lcd.setCursor(0,1);
               lcd.print(F("P_Return : "));
