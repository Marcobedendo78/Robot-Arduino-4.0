...
void Print_LCD_Menu_Docked(byte LCD_Menu_Docked) {
  if (LCD_Menu_Docked == 1) lcd.print(F("Uscita Zona Z-1"));
  if (LCD_Menu_Docked == 2) lcd.print(F("Uscita Zona Z-2"));
  if (LCD_Menu_Docked == 3) lcd.print(F("Partenza Rapida"));
  if (LCD_Menu_Docked == 4) lcd.print(F("Taglia Sul Filo"));
  if (LCD_Menu_Docked == 5) lcd.print(F("Test Tagliaerba"));
  if (LCD_Menu_Docked == 6) lcd.print(F("Setup Tagiaerba"));

  Max_Options_Docked = 6;
  }
...
void Activate_Menu_Option_Docked() {
     ...
     if (Menu_Mode_Selection == 1) {
       ...
       lcd.clear();
       lcd.print("Avvio Manuale");
       lcd.setCursor(0,1);
       lcd.print("Uscita Zona Z1");
     ...
     if (Menu_Mode_Selection == 3) {
      ...
      if (Menu_Mode_Selection == 4) {
       lcd.clear();
       lcd.print("Lama Accesa");
       lcd.setCursor(0,1);
       lcd.print("Taglia sul Filo");
       delay(1000);
       lcd.clear();
       Print_Membrane_Switch_Input_Timing();                             // Changes the menu to select the mow time
       if (Mow_Time_Set == 1) {
