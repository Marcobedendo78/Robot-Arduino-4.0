...
void Track_Perimeter_Wire_To_Dock()  {
  ...
  Serial.print(F("P_Return = "));
  Serial.print(P_Return);
  Serial.print(F(" / D = "));
  Serial.print(D);
  Serial.println(F(""));
  ...
  if (CCW_Tracking_To_Charge == 1)  {                                            // Il tosaerba segue il filo in senso antiorario
    ...
    // 🔽 MODIFICA: rallentamento progressivo in base alla forza del segnale
    int signalStrength = abs(MAG_Start);
    if (signalStrength >= 80) {
      PWM_TrackSpeed_LH = 120;                                                    // Molto vicino → rallenta al minimo
      PWM_TrackSpeed_RH = 120;                                                    // Molto vicino → rallenta al minimo
    }

      MAG_Error = (MAG_Goal - MAG_Start);                                         // Calcola l'errore al centro del filo che normalmente è di magnitudine zero (ricorda - - è + )
      if (abs(MAG_Error) < 10) MAG_Error = 0;                                     // Zona morta per evitare oscillazioni
      PrintWirePosition();                                                        // Stampa la panoramica sul monitor seriale.
      ...
      if (MAG_Error > 0) {                                                        // Ruotare il tosaerba a destra di MAG Error > 0 con una direzione di marcia antioraria.
        ...
        PWM_Right = PWM_TrackSpeed_RH - (MAG_Error * P_Return);                          // Mag_Error * P è il valore ridotto dal valore massimo impostato PWM e inviato al PWM                 
      ...
      if (MAG_Error <= 0) {                                                       // Girare il tosaerba a sinistra se MAG_Error < 0 con una direzione di marcia antiorariaon
        ...
        PWM_Left = PWM_TrackSpeed_LH + (MAG_Error * P_Return);                           // + poiché mag_error è negativo per regolare PWM
      ...
      if (Dock_Cycles > 10) {
        ...
        lcd.setCursor(10,1);
        lcd.print(I);
        Check_Wheel_Amps();                                                         // Nuova aggiunta 15/06/2024
        if (Wheel_Blocked == 4) {                                                   // Nuova aggiunta 15/06/2024
          Serial.println(F("Le ruote del tagliaerba sono inceppate"));              // Nuova aggiunta 15/06/2024
          Compass_Turn_Mower_To_Home_Direction();                                   // Nuova aggiunta 15/06/2024
          Manouver_Find_Wire_Track();                                               // Nuova aggiunta 15/06/2024
         } 
    }
  ...
  if (CW_Tracking_To_Charge == 1)  {                                                  // Il tosaerba segue il filo in senso antiorario
    ...
    if (signalStrength >= 80) {
      ...
      if (MAG_Error <= 0) {                                                            // Girare il tosaerba a destra per tornare sul cavo.
        //GIRA A DX
        PWM_Left = PWM_TrackSpeed_RH;
        PWM_Right = PWM_TrackSpeed_RH + (MAG_Error * P_Return);                               // + poiché mag_error è negativo per regolare PWM
        if (PWM_Right > PWM_TrackSpeed_RH) PWM_Right = PWM_TrackSpeed_RH;
        if (PWM_Right >= 0) {
          SetPins_ToGoForwards();
          lcd.setCursor(15, 0);
