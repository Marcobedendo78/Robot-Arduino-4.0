...
void Print_LCD_Menu_Parked(byte LCD_Menu_Parked) {
  if (LCD_Menu_Parked == 1)   lcd.print("Taglio Erba     ");
  if (LCD_Menu_Parked == 2)   {
    if (Use_Charging_Station == 1)  lcd.print("Vai alla Base");
    if (Use_Charging_Station == 0)  lcd.print("Base Rimossa");
    }
  if (LCD_Menu_Parked == 3)   lcd.print(F("Test Tagliaerba"));
  if (LCD_Menu_Parked == 4)   lcd.print(F("Setup Tagliaerba"));

  Max_Options_Parked = 5;
  }
