...
void Print_LCD_Menu_Timing(byte LCD_Menu_Timing) {
  if (LCD_Menu_Timing == 1) lcd.print(F("Taglio Massimo"));
  if (LCD_Menu_Timing == 2) lcd.print(F("Taglia per 1hr "));
  if (LCD_Menu_Timing == 3) lcd.print(F("Taglia per 2hr"));
  if (LCD_Menu_Timing == 4) lcd.print(F("Taglia per 3hr"));
  if (LCD_Menu_Timing == 5) lcd.print(F("Taglia per 4hr"));                  //leave blank
  Max_Options_Timing = 5;
 }  
...
void Activate_Menu_Option_Timing() {
     ...
     if (Menu_Mode_Selection == 1) {
       ...
       lcd.clear();
       lcd.print(F("Taglio Massimo"));
       lcd.setCursor(0,1);
       lcd.print(F("Selezionato"));
     ...
     if (Menu_Mode_Selection == 3) {
       ...
       // 3hr Mowing
       Serial.println(F("4 hr Mow Time Selected"));
       lcd.clear();
       Menu_Mode_Selection = 0;
       lcd.clear();
       Menu_Complete = false;
           ...
           Time t = rtc.time();
           Alarm_Timed_Mow_ON = 1;                          // Activate the Mow Timer Alarm
           Alarm_Timed_Mow_Hour = t.hr + 3;                 // Sets time to 1 hour later.
           Alarm_Timed_Mow_Minute = t.min;                  // Minutes are the same
     ...
     if (Menu_Mode_Selection == 5) {
       ...
       Menu_Complete = false;
           ...
           Time t = rtc.time();
           Alarm_Timed_Mow_ON = 1;                          // Activate the Mow Timer Alarm
           Alarm_Timed_Mow_Hour = t.hr + 4;                 // Sets time to 1 hour later.
           Alarm_Timed_Mow_Minute = t.min;                  // Minutes are the same
