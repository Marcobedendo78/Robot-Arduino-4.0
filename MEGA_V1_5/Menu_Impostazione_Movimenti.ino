   ...
   if (Menu_Mode_Selection == 3) {
     ...
     if (Menu_Mode_Selection == 8) {
      ...
      while (!Menu_Complete_Motion) {
        ...
        // Salva in EEPROM
        EEPROM.write(171, 1); EEPROM.write(170, int(Kp * 100));
        EEPROM.write(173, 1); EEPROM.write(172, int(Ki * 100));
        EEPROM.write(175, 1); EEPROM.write(174, int(Kd * 100));
        Menu_Complete_Motion = true;
        Menu_Mode_Selection = 0;
      } else {
        Set++;
        lcd.clear();
        if (Set == 2) {
          lcd.setCursor(0,0); lcd.print(F("  Kp: ")); lcd.print(Kp);
          lcd.setCursor(0,1); lcd.print(F("> Ki: ")); lcd.print(Ki);
        } else if (Set == 3) {
          lcd.setCursor(0,0); lcd.print(F("  Ki: ")); lcd.print(Ki);
          lcd.setCursor(0,1); lcd.print(F("> Kd: ")); lcd.print(Kd);
        }
      }
    }

    if (!Plus_Key_X) {
      if (Set == 1) { Kp += 0.01; if (Kp > 2.55) Kp = 2.55; }
      if (Set == 2) { Ki += 0.01; if (Ki > 2.55) Ki = 2.55; }
      if (Set == 3) { Kd += 0.01; if (Kd > 2.55) Kd = 2.55; }
    }

    if (!Minus_Key_X) {
      if (Set == 1) { Kp -= 0.01; if (Kp < 0) Kp = 0; }
      if (Set == 2) { Ki -= 0.01; if (Ki < 0) Ki = 0; }
      if (Set == 3) { Kd -= 0.01; if (Kd < 0) Kd = 0; }
    }

    // aggiorna display
    lcd.clear();
    if (Set == 1) {
      lcd.setCursor(0,0); lcd.print(F("> Kp: ")); lcd.print(Kp, 2);
      lcd.setCursor(0,1); lcd.print(F("  Ki: ")); lcd.print(Ki, 2);
    }
    if (Set == 2) {
      lcd.setCursor(0,0); lcd.print(F("  Kp: ")); lcd.print(Kp, 2);
      lcd.setCursor(0,1); lcd.print(F("> Ki: ")); lcd.print(Ki, 2);
    ...
    if (Set == 3) {
      lcd.setCursor(0,0); lcd.print(F("  Ki: ")); lcd.print(Ki, 2);
      lcd.setCursor(0,1); lcd.print(F("> Kd: ")); lcd.print(Kd, 2);
    }
  }
}
