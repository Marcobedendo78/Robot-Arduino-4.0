...
void Print_LCD_Menu_Sensors(byte LCD_Menu_Sensors) {
  ...
  if (LCD_Menu_Sensors == 2) lcd.print(F("Distanza SONAR"));
  if (LCD_Menu_Sensors == 3) lcd.print(F("Sensib. SONAR"));
  if (LCD_Menu_Sensors == 4) lcd.print(F("Batteria Min/Max"));
  if (LCD_Menu_Sensors == 5) lcd.print(F("Batt Sensibilità"));
  if (LCD_Menu_Sensors == 6) lcd.print(F("FILO ON/OFF"));
  if (LCD_Menu_Sensors == 7) lcd.print(F("PIOGGIA ON/OFF"));
...
void Activate_Menu_Option_Sensors() {
    ...
    while (Menu_Complete_Sensors == false) {
      ...
      if (!Start_Key_X) {
        ...
        if (Set == 2) {
          lcd.clear();
          lcd.setCursor(0, 0);
          lcd.print(F("S = Si  N = No"));
          lcd.setCursor(0, 1);
          lcd.print(F(" S2:"));
          if (Sonar_2_Activate == 1) lcd.print("S");
          if (Sonar_2_Activate == 0) lcd.print("N");
          lcd.print(F(" S1:"));
          if (Sonar_1_Activate == 1) lcd.print("S");
          if (Sonar_1_Activate == 0) lcd.print("N");
          lcd.print(F(">S3:"));
          if (Sonar_3_Activate == 1) lcd.print("S");
          if (Sonar_3_Activate == 0) lcd.print("N");
          Set = Set + 1;
        }
  ...
  if (Menu_Mode_Selection == 3) {
    ...
    delay(500);
    lcd.setCursor(0, 0);
    lcd.print(F("Sensib. Sonar:"));
    lcd.setCursor(0, 1);
    lcd.print(F("Hits = "));
    lcd.print(Max_Sonar_Hit);
    Serial.print(F("Sensib Sonar = :"));
  ...
  if (Menu_Mode_Selection == 5) {
    ...
    lcd.clear();
    lcd.setCursor(0, 0);
    lcd.print(F("Sensibilità Batt"));
    lcd.setCursor(0, 1);
    lcd.print(F("Hits = "));
    lcd.print(Low_Battery_Instances_Chg);
    Serial.print(F("Sensib Batt = :"));
  ...
  if (Menu_Mode_Selection == 8) {
    ...
    lcd.clear();
    lcd.setCursor(0, 0);
    lcd.print(F("Sensib. Pioggia:"));
    lcd.setCursor(0, 1);
    lcd.print(F("Hits = "));
    lcd.print(Rain_Total_Hits_Go_Home);
    Serial.print(F("Sens Pioggia = :"));
  ...
  if (Menu_Mode_Selection == 9) {
    ...
    lcd.setCursor(0, 1);
    lcd.print(F("Stato: "));
    if (WIFI_Enabled == 1) lcd.print(F("Attivato "));
    if (WIFI_Enabled == 0) lcd.print(F("Disattivato"));
