...
void Manouver_Mow_The_Grass() {
  ...
  // --- variabili statiche per lo stato del run per la rampa di accelerazione ---
  static int  lastCycle = -1;                                        // per rilevare l'inizio di un nuovo run (Loop_Cycle_Mowing torna a 0)
  static bool ramp_done_this_run = false;                            // rampa eseguita in questo run?
  ...
  // Rampa di accelerazione non bloccante (solo una volta per run)
  if (!ramp_done_this_run) {
    if (!Motor_Action_SoftStart_IsActive()) {
      Motor_Action_SoftStart_To_Mow(RAMP_TIME_MS);                   // avvia rampa (tuning 600–1500 ms)
    } else {
      Motor_Action_SoftStart_To_Mow(0);                              // aggiorna rampa
      if (!Motor_Action_SoftStart_IsActive()) {
        ramp_done_this_run = true;                                   // rampa conclusa per questo run
      }
    }

    // Finché la rampa è attiva, non eseguire altro che possa toccare i PWM.
    if (!ramp_done_this_run) {
      // Early-return: evita di riscrivere PWM con altre funzioni
      lastCycle = Loop_Cycle_Mowing;
      Loop_Cycle_Mowing++;
      return;                                                        // evita che il codice sotto riscriva i PWM durante la rampa
    }
  }

  // ----- DA QUI IN POI: controllo normale (rampa finita per questo run) -----
    ...
    if (Loop_Cycle_Mowing == 3) {
        if (Pattern_Mow == 0) {
            if ((Compass_Heading_Hold_Enabled == 1) && (GYRO_Enabled == 1)) {
                ...
                Motor_Action_Go_Full_Speed();
                Serial.println(F("Gyro non attivato"));
    ...
    if (Loop_Cycle_Mowing > Max_Cycles_Active) {
        ...
        Loop_Cycle_Mowing = 0;
        lcd.clear();
        // ramp_done_this_run verrà ri-armato automaticamente al prossimo ingresso
        // grazie alla detection "Loop_Cycle_Mowing == 0 && lastCycle != 0"
    }
}
...
void Manouver_Turn_Around() {
    ...
    lastTime = millis();                                                  // Reset tempo per deltaTime
    lcd.clear();
    // --- NUOVO RUN: prepara la rampa ---
    analogWrite(ENAPin, 0);                                               // azzera subito PWM motore DX
    analogWrite(ENBPin, 0);                                               // azzera subito PWM motore SX
    Compass_Heading_Locked = 0;                                           // Reset Compass Heading Lock
    delay(500);
}
