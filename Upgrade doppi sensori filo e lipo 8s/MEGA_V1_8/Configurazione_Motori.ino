...
void Motor_Action_Max_Slow_Speed_3()     {
      analogWrite(ENAPin,  PWM_Max_Slow_Speed_3_RH);                  // Velocità = 0-255  (255 è la velocità massima). La velocità è impostata nelle impostazioni                        
      analogWrite(ENBPin,  PWM_Max_Slow_Speed_3_LH);                  // AnaolgWrite invia segnali PWM Velocità = 0-255  (255 è la velocità massima)
      Serial.print(F("Wheel:MAX_SLOW|"));
      }
...
//Rampa di Accelerazione/Crocera/Decelerazione per le inversioni e manovre
void Motor_Action_Go_Accel(unsigned long durataTotale, float percentualeAccel = 0.4, float percentualeDecel = 0.4) {
  ...
  int stepsAccel = max(PWM_Max_Slow_Speed_2_RH, PWM_Max_Slow_Speed_2_LH);
  ...
  for (int i = 0; i <= stepsAccel; i++) {
    int currentRH = min(i, PWM_Max_Slow_Speed_2_RH);
    int currentLH = min(i, PWM_Max_Slow_Speed_2_LH);
  ...
  // Crociera
  analogWrite(ENAPin, PWM_Max_Slow_Speed_2_RH);
  analogWrite(ENBPin, PWM_Max_Slow_Speed_2_LH);
...
// Rampa dolce fino alla velocità di taglio (una sola volta a inizio ciclo)
static bool      g_softStartActive = false;
static uint32_t  g_softStartT0     = 0;
static uint32_t  g_softStartDur    = 0;
static uint8_t   g_targetRH        = 0;
static uint8_t   g_targetLH        = 0;

void Motor_Action_SoftStart_To_Mow(unsigned long durata_ms) {
  // Avvio rampa (prima chiamata)
  if (!g_softStartActive) {
    g_targetRH = PWM_MaxSpeed_RH;
    g_targetLH = PWM_MaxSpeed_LH;

    if (durata_ms == 0 || (g_targetRH == 0 && g_targetLH == 0)) {
      analogWrite(ENAPin, g_targetRH);
      analogWrite(ENBPin, g_targetLH);
      return;
    }
    g_softStartActive = true;
    g_softStartT0     = millis();
    g_softStartDur    = durata_ms;

    analogWrite(ENAPin, 0);
    analogWrite(ENBPin, 0);
    Serial.print(F("Wheel:SOFT_START_BEGIN|"));
    return; // inizializzato; gli step li facciamo nelle chiamate successive
  }

  // Aggiornamento rampa
  uint32_t elapsed = millis() - g_softStartT0;

  // clamp a fine rampa: imposta direttamente i target e chiudi
  if (elapsed >= g_softStartDur) {
    analogWrite(ENAPin, g_targetRH);
    analogWrite(ENBPin, g_targetLH);
    g_softStartActive = false;
    Serial.print(F("Wheel:SOFT_START_END|"));
    return;
  }

  uint8_t currRH, currLH;

#if SOFTSTART_EASE_QUAD
  // Easing "ease-in" quadratico con interi:
  // progress = (elapsed/dur)^2  -> PWM = target * progress
  // Usiamo 64 bit per evitare overflow in elapsed*elapsed
  uint64_t e  = (uint64_t)elapsed;
  uint64_t d  = (uint64_t)g_softStartDur;
  uint64_t pN = e * e;      // numeratore progress
  uint64_t pD = d * d;      // denominatore progress
  currRH = (uint8_t)(( (uint64_t)g_targetRH * pN ) / pD);
  currLH = (uint8_t)(( (uint64_t)g_targetLH * pN ) / pD);
#else
  // Lineare puro con soli 32 bit:
  // PWM = target * elapsed / durata
  currRH = (uint8_t)(((uint32_t)g_targetRH * elapsed) / g_softStartDur);
  currLH = (uint8_t)(((uint32_t)g_targetLH * elapsed) / g_softStartDur);
#endif

  analogWrite(ENAPin, currRH);
  analogWrite(ENBPin, currLH);
}

bool Motor_Action_SoftStart_IsActive() { return g_softStartActive; }
