void Check_Wire_In_Out() {
  ...
  if (Outside_Wire_Count >= Outside_Wire_Count_Max) {
    ...
    if  (Action_On_Over_Wire_Count_Max == 3) {                                            // prova a individuare il filo utilizzando la funzione di ricerca filo
      ...
      //Motor_Action_Go_Accel(800);                                       //Prima era 700 15/06/22
...
void TestforBoundaryWire()  {
  ...
  if (Perimeter_Wire_Enabled == 1) {                                               // L'uso del perimetro è ATTIVO: Perimter_USE può essere attivato o disattivato nella configurazione.
...
// Legge ogni 50 ms se perimeter.isInside è 0 (Fuori) o 1 (Dentro) sensore SX
void UpdateWireSensor()   {
    if (millis() >= nextTime)  {
    nextTime = millis() + 50;                  // Attendi 50 ms prima di leggere di nuovo
     if (perimeter.isInside(0) != inside) {    // Canale sinistro
       inside = perimeter.isInside(0);         // Aggiorna stato
       counter++;                              // Conta il cambiamento
     }
   }
}

// Legge ogni 50 ms se perimeter.isInside è 0 (Fuori) o 1 (Dentro) sensore DX
void UpdateWireSensorRight() {
  if (millis() >= nextTimeR) {
    nextTimeR = millis() + 50;                 // Attendi 50 ms prima di leggere di nuovo
    bool inNow = perimeter.isInside(1);        // Canale destro
    if (inNow != insideR) {
      insideR = inNow;
      counterR++;                              // Conta il cambiamento
    }
  }
}

// Lettura immediata se perimeter.isInside è 0 (Fuori) o 1 (Dentro) nella rotazione sul filo sensore SX
void UpdateWireSensor2() {
...
// Lettura immediata se perimeter.isInside è 0 (Fuori) o 1 (Dentro) nella rotazione sul filo  sensore DX
void UpdateWireSensorRight2() {
  if (perimeter.isInside(1) != insideR) {
    insideR = perimeter.isInside(1);            // Aggiorna stato
    counterR++;                                 // Conta il cambiamento
  }
}
