...
// Versione con Driver motore Lama ZS-X11H (motore brushless) o BTS7960 (motore a spazzole)         //
// Versione con RPlidar A1 M8                                                                       //
// Versione con doppio sensore filo perimetrale e batteria fino a massimo lipo 8s                   //
// Questa versione richiede la modifica al sensore di tensione                                      //
// sostituendo la resistenza da 30kΩ con 75kΩ                                                       //
// Quella con la sigla 3002 va sostituita                                                           //
// Va aggiornato di conseguenza anche lo sketch del Arduino Nano e del NodeMCU                      //
//////////////////////////////////////////////////////////////////////////////////////////////////////

/*
Selezione tra driver ZS-X11H o BTS7960.
Impostare a (1) per usare e a (0) per non usare
*/
#define USE_DRIVER_ZS_X11H      1   // imposta a 1 per usare ZS-X11H
#define USE_DRIVER_BTS7960      0   // imposta a 1 per usare BTS7960

#if (USE_DRIVER_ZS_X11H == USE_DRIVER_BTS7960)
  #error "Devi abilitare solo uno tra USE_DRIVER_ZS_X11H e USE_DRIVER_BTS7960"
#endif

//Librerie per ricevitore filo perimetrale
#include <Arduino.h>
#include <Wire.h>
#include <EEPROM.h>
#include "drivers.h"
...
//Librerie per orologio in tempo reale
#include <stdio.h>
#include <DS1302.h>

//Librerie per ic2 Liquid Crystal
...
#define Relay_Motors 24         
  ...
  long    GYRO_Angle;                  // Aggiornato
  const int MPU_addr = 0x69;
  int16_t GyZ;
  long    Gyro_Heading = 0;           // in centesimi di grado
  long    gyroZrate = 0;              // in centesimi di °/s
  int     gyroBias = 0;
  unsigned long lastTime = 0;
  unsigned long lastGyroRead = 0;
  const unsigned long gyroInterval = 50;
  ...
  int   Ki_EEPROM;
  int   Kd_EEPROM;
  float Tracking_PID_P_Return_EEPROM;
...
*******************************************************************************************************************************************/
  ...
  // Tracciatura Filo
  int Track_Wire_Zone_1_Cycles    = 500;    //EEPROM            // Zona 1 - Numero di iterazioni eseguite dalla funzione PID prima che il rasaerba esca dalla tracciatura del filo perimetrale.
  int Track_Wire_Zone_2_Cycles    = 7000;   //EEPROM            // Zone 2 - Numero di iterazioni eseguite dalla funzione PID prima che il rasaerba esca dalla tracciatura del filo perimetrale.
  int Track_Wire_Zone_3_Cycles    = 10000;  //EEPROM            // Falciatura lungo il filo perimetrale.
  ...
  int Cycles_Max                  = 10000;  //EEPROM            // Valore massimo che il robot segue il filo prima di eseguire la manovra per sganciari da un'aiuola e ripetere la ricerca del filo peimetrale.
  ...
  // Settaggi Batteria
  float Battery_Max               = 33.6;                       // Volt massimi selezionabili della batteria in Volt. 8S = 33.6V (4.2v per cella).
  float Battery_Min               = 14.6;   //EEPROM            // Limite inferiore di carica della batteria prima della ricarica richiesta.
                                                                // 4S = 14.8V (3.7v per cella) circa 50%. Scarica a 20% = 14.5 (3.625V per cella). Scarica a 0% = 13.2 (3.3V per cella).
  ...
  // Paraurti
  bool Bumper_Activate_Frnt       = 0;      //EEPROM            // Attiva il paraurti anteriore.
  ...
  int PWM_TrackSpeed_LH            = 140;     //EEPROM           // Velocità del tosaerba durante la navigazione sul filo perimetrale ruota SX.
  int PWM_TrackSpeed_RH            = 140;     //EEPROM           // Velocità del tosaerba durante la navigazione sul filo perimetrale ruota DX.
  int PWM_Max_Slow_Speed_LH        = 120;                        // Velocità del tosaerba durante il posizionamento avanti sul filo ruota SX.
  int PWM_Max_Slow_Speed_RH        = 120;                        // Velocità del tosaerba durante il posizionamento avanti sul filo ruota DX.
  int PWM_Max_Slow_Speed_2_LH      = 120;                        // Velocità del tosaerba durante la ricerca del filo e manovre con rampa ruota SX.
  int PWM_Max_Slow_Speed_2_RH      = 120;                        // Velocità del tosaerba durante la ricerca del filo e manovre con rampa ruota DX.
  int PWM_Max_Slow_Speed_3_LH      = 100;                        // Velocità del tosaerba durante la rotazione sul filo ruota SX.
  int PWM_Max_Slow_Speed_3_RH      = 100;                        // Velocità del tosaerba durante la rotazione sul filo ruota DX.
  int PWM_Compass_Speed_LH         = 120;                        // Velocità del tosaerba durante la ricerca della direzione per tovare il filo ruota SX.
  int PWM_Compass_Speed_RH         = 120;                        // Velocità del tosaerba durante la ricerca della direzione per tovare il filo ruota DX.
  const unsigned long RAMP_TIME_MS = 4000UL;                     // Durata della rampa di accelerazione in millisecondi (2000=2 secondi) ad ogni partenza.
                                                                 // Dopo il valore mettere "UL"
  #define SOFTSTART_EASE_QUAD 1                                  // opzionale: abilita easing quadratico per la rampa di accelerazione ad ogni partenza (0 = lineare, 1 = ease-in)
  ...
  float Max_Wheel_Amps             = 2.4;                        // Amperaggio massimo consentito nelle ruote prima che venga chiamato un blocco.
  ...
  // Parametri PID mantenimento direzione (regolabili)
  int Kp                           = 10;     //EEPROM            // Kp troppo basso? il robot corregge lentamente - Kp troppo alto? il robot “balla” a zig-zag
  int Ki                           = 0;      //EEPROM            // Ki alto? rischi accumulo e reazioni lente
  int Kd                           = 0;      //EEPROM            // Kd troppo alto? il robot sovracompensa
  ...
  */
  ...
  // Valori di rilevamento filo
    ...
    float P               = 0.60;     //EEPROM                     // Fattore di moltiplicazione per l'errore misurato al centro del filo. Se il movimento è a scatti durante il monitoraggio riduce il numero
                                                                   // Valore per la tracciatura del filo perimetrale in uscita dalla base
    float P_Return        = 0.62;     //EEPROM                     // Fattore di moltiplicazione per l'errore misurato al centro del filo. Se il movimento è a scatti durante il monitoraggio riduce il numero
                                                                   // Valore per la tracciatura del filo perimetrale nel ritorno alla base
    ...
    int Outside_Wire_Count_Max          = 16;                      // Se il tagliaerba è fuori dal cavo molte volte, il tagliaerba si ferma
    ...
    int Max_Spin_Attempts             = 100;                       //Rotazione sul filo per ritorno alla base
...
void setup() {
  ...
  if (RPLIDAR_Enabled == true) Serial3.begin(115200);     // Se il Lidar è attivo prepara la porta seriale 3 aperta per la comunicazione per RPLidar
  lidar.begin(Serial3);
...
// === Lettura Gyro ===
  if (millis() - lastGyroRead >= gyroInterval) {
    lastGyroRead = millis();
    Get_GYRO_Reading();  // mantiene aggiornato Gyro_Heading
  }
...
if ((Mower_Running == 1) && (Wire_Detected == 1) && (Outside_Wire == 0) && (Sonar_Hit == 0))                              Check_Bumper();                     // Se il paraurti del tagliaerba è il cavo perimetrale sono attivi, controllare l'attivazione del paraurti.
...
if ((Mower_Running == 1) && (Wire_Detected == 1) && ((Outside_Wire == 1) || (Bumper == 1)))                               Manouver_Turn_Around();             // Se il paraurti è attivato o il tosaerba è fuori dal cavo perimetrale, girarsi.
if ((Mower_Running == 1) && (Wire_Detected == 1) && (Outside_Wire == 0) && (Sonar_Hit == 1))                              Manouver_Turn_Around_Sonar();       // Se viene rilevato un colpo del sonar è il cavo, il tosaerba fa la manovra per aggirare l'ostacolo. 
if ((Mower_Running == 1) && (Wire_Detected == 1) && (Outside_Wire == 0) && (RPLIDAR_Enabled == 1))                        Check_LIDAR();                      // Se il tosaerba è in funzione, se il cavo è rilevato, se il tosaerba è dentro il perimetro del filo,
                                                                                                                                                              // e se il LIDAR è attivato, allora viene eseguita le funzioni del LIDAR.
